...
static void binhlong(), binhulong(), binhdouble(), binhconst(), binhllong(), binhullong();
...
void (*BINFUN[]) () = {
    ...
    binhllong, binhullong,  /* t_llong, t_ullong */
...
#define SETUNUM  I(t_uchar)+I(t_ushort)+I(t_uint)+I(t_ulong)+I(t_ullong)
	     ...
	     token_t * e2, int explicit)
{
	...
	case t_llong: case t_ullong: opcode = long2llong; break;
	case t_pointer:
	...
	CASE_LONG: CASE_ULONG:
		...
		opcode =  llong2long;
	    break;
	case t_llong: case t_ullong: break;
	case t_pointer:
	...
	case t_llong: case t_ullong: break;
	case t_pointer:
	    convert2ptr(e1,ullong2ptr, explicit,
#ifndef NO_LONGLONG			
			sizeof(long long));
#else
	                sizeof(long));
...
void EiC_castconst(token_t * e1, token_t * e2, int explicit)
{
	...
	switch (EiC_gettype(e1->Type)) {
	  ...
	  case t_ullong: e1->Val.ival = (int)e1->Val.ullval; break;
	  default:
	    EiC_error("Illegal cast operation");
	}
	break;
	...
	case t_llong:e1->Val.lval = (long)e1->Val.llval;
	  break;
	case t_ullong:e1->Val.lval = (long)e1->Val.ullval;
	  break;
	...
	switch (EiC_gettype(e1->Type)) {
	  ...
	  case t_ulong:
	    return;
	case t_llong: e1->Val.ulval = (unsigned long)e1->Val.llval; break;
	case t_ullong: e1->Val.ulval = (unsigned long)e1->Val.ullval; break;
	...
	break;
      ...
      case t_ullong:
	switch (EiC_gettype(e1->Type)) {
	  CASE_INT:    e1->Val.ullval =  (eic_ullong) e1->Val.ival;   break;
	  CASE_UINT:   e1->Val.ullval =  (eic_ullong) e1->Val.uival; break;
	  CASE_LONG:   e1->Val.ullval =  (eic_ullong) e1->Val.lval;  break;
	  case t_ulong:e1->Val.ullval =(eic_ullong) e1->Val.ulval;  break;
	  case t_llong: case t_ullong: break;

	  CASE_FLOAT:e1->Val.ullval = (eic_ullong)e1->Val.dval;
	    break;
	  default:
	    EiC_error("Illegal cast operation");
	}
	...
	case t_ullong: e1->Val.dval = (double)e1->Val.ullval; break;
	case t_pointer:
...
void binhullong(unsigned oper, token_t * e1, token_t * e2)
{
    checkllong(t_ullong, e1, e2);
    
    switch (oper) {
      case '+':
	EiC_generate(&e2->Code, addullong, &e2->Val, 0);
	break;
      case '-':
	EiC_generate(&e2->Code, subullong, &e2->Val, 0);
	break;
      case '*':
	EiC_generate(&e2->Code, multullong, &e2->Val, 0);
	break;
      case '/':
	EiC_generate(&e2->Code, divullong, &e2->Val, 0);
	break;
      case '%':
	EiC_generate(&e2->Code, modullong, &e2->Val, 0);
	break;
      case LSHT:
	EiC_generate(&e2->Code, lshtullong, &e2->Val, 0);
	break;
      case RSHT:
	EiC_generate(&e2->Code, rshtullong, &e2->Val, 0);
	break;
      case BOR:
	EiC_generate(&e2->Code, borullong, &e2->Val, 0);
	break;
      case XOR:
	EiC_generate(&e2->Code, xorullong, &e2->Val, 0);
	break;
      case AND:
	EiC_generate(&e2->Code, andullong, &e2->Val, 0);
	break;
      default:
	EiC_output(e1);
	EiC_exchtype(t_int, e1->Type);	/* should really be boolean */
	switch (oper) {
	  case LT:
	    EiC_generate(&e2->Code, ltullong, &e2->Val, 0);
	    break;
	  case LE:
	    EiC_generate(&e2->Code, leullong, &e2->Val, 0);
	    break;
	  case EQ:
	    EiC_generate(&e2->Code, equllong, &e2->Val, 0);
	    break;
	  case NE:
	    EiC_generate(&e2->Code, neullong, &e2->Val, 0);
	    break;
	  case GT:
	    EiC_generate(&e2->Code, gtullong, &e2->Val, 0);
	    break;
	  case GE:
	    EiC_generate(&e2->Code, geullong, &e2->Val, 0);
	    break;
	}
	break;
    }
...
void binhdouble(unsigned oper, token_t * e1, token_t * e2)
{
	...
	if (!isconst(e1->Type)) {
		...
		break;
	    case t_ullong: EiC_generate(&e1->Code, ullong2double, &e1->Val, 0);
		break;
	    }
	...
	if (!isconst(e2->Type)) {
		...
		break;
	      case t_ullong:EiC_generate(&e2->Code, ullong2double, &e1->Val, 0);
		break;
	    }
...
int EiC_unaryop(token_t * e1, int op)
{
	...
	switch(op) {
	  case '-':
		...
		break;
	      case t_llong:
		e1->Val.llval = -e1->Val.llval;
		break;
	      case t_ullong:
		e1->Val.ullval = -e1->Val.ullval;
		break;
	      CASE_FLOAT:
	  ...
	  case '~':
		...
		break;
	    case t_llong:
		e1->Val.llval = ~e1->Val.llval; break;
	    case t_ullong:
		e1->Val.ullval = ~e1->Val.ullval;
		EiC_exchtype(t_ullong,e1->Type);
		setConst(e1->Type);
		break;
	      default:
		EiC_error("Illegal operand");
	...
	switch ((t =EiC_gettype(e1->Type))) {
	  ...
	  case t_ulong:
	    ...
	    break;
	  case t_llong:
	    EiC_generate(&e1->Code, negllong, &e1->Val, 0);
	    break;
	  case t_ullong:
	    EiC_generate(&e1->Code, negullong, &e1->Val, 0);
	    break;
	  CASE_FLOAT:
	...
	switch ((t=EiC_gettype(e1->Type))) {
	  ...
	  case t_llong:
	  case t_ullong:
	    EiC_generate(&e1->Code, compullong, &e1->Val, 0);
	    EiC_exchtype(t_ullong,e1->Type);
	    break;
	  default:
	    EiC_error("Illegal operand");
	...
	switch (EiC_gettype(e1->Type)) {
	  ...
	  case t_llong:
	  case t_ullong:
	    EiC_generate(&e1->Code, notllong, &e1->Val, 0);
	    break;
...
int do_binaryop(token_t * e1, token_t * e2, int op)
{
	...
	switch (op) {
	  ...
	  case RSHT:
	    ...
	    break;
	  case AND:
	    e1->Val.llval = e1->Val.llval & e2->Val.llval;
	    break;
	  case BOR:
	    e1->Val.llval = e1->Val.llval | e2->Val.llval;
	    break;
	  ...
	  default:
	    ...
	    switch (op) {
	      case LT:
		e1->Val.ival = e1->Val.llval < e2->Val.llval;
		break;
	      case LE:
		e1->Val.ival = e1->Val.llval <= e2->Val.llval;
		break;
	...
	switch (op) {
	  ...
	  case RSHT:
	    ...
	    break;
	  case AND:
	    e1->Val.ullval = e1->Val.ullval & e2->Val.ullval;
	    break;
	  case BOR:
	    e1->Val.ullval = e1->Val.ullval | e2->Val.ullval;
	    break;
	  ...
	  default:
	    ...
	    switch (op) {
	      case LT:
		e1->Val.ival = e1->Val.ullval < e2->Val.ullval;
		break;
	      case LE:
		e1->Val.ival = e1->Val.ullval <= e2->Val.ullval;
		break;
...
void * EiC_getaddress(token_t * e1)
{
	...
	case t_union:
	case t_struct:
	    EiC_error("Illegal use of structure or union: need an lvalue");
	case t_array:
	    if (t == t_array)
		EiC_exchtype(t_pointer, e1->Type);
	case t_pointer:
...
void EiC_output(token_t * e1)
{
	...
	if(isconst(e1->Type)) {
		...
		break;
	    case t_ullong: EiC_generate(&e1->Code, pushullong, &e1->Val, 0);
		break;
	...
	/* EiC-Improved only: volatile rval must be accessed through dereferencing */
	if(!__backward__ && t >= t_char && t <= t_ullong && isvolatile(e1->Type)) 
	  EiC_warningerror("Suspicious rval with volatile qualifier");
	...
	CASE_ULONG:EiC_generate(&e1->Code, rvallong, &e1->Val, EiC_GI(e1));
	    ...
	    case t_lval:
		e1->Type = EiC_succType(e1->Type);
		volqual = isvolatile(e1->Type) && !__backward__;
		switch (EiC_gettype(e1->Type)) {
		    case t_uchar:
			EiC_generate(&e1->Code, volqual ? drefvoluchar : drefuchar, &e1->Val, 0);
			break;
		    case t_char:
			EiC_generate(&e1->Code, volqual ? drefvolchar : drefchar, &e1->Val, 0);
			break;
		    case t_short:
			EiC_generate(&e1->Code, volqual ? drefvolshort : drefshort, &e1->Val, 0);
			break;
		    case t_ushort:
			EiC_generate(&e1->Code, volqual ? drefvolushort : drefushort, &e1->Val, 0);
			break;
		    case t_int:
			EiC_generate(&e1->Code, volqual ? drefvolint : drefint, &e1->Val, 0);
			break;
		    case t_uint:
			EiC_generate(&e1->Code, volqual ? drefvoluint : drefuint, &e1->Val, 0);
			break;
		CASE_LONG:EiC_generate(&e1->Code, volqual ? drefvollong : dreflong, &e1->Val, 0);
		break;
		CASE_ULONG:EiC_generate(&e1->Code, volqual ? drefvolulong : drefulong, &e1->Val, 0);
		break;
		    case t_llong:EiC_generate(&e1->Code, volqual ? drefvolllong : drefllong, &e1->Val, 0);
			break;
		    case t_ullong:EiC_generate(&e1->Code, volqual ? drefvolullong : drefullong, &e1->Val, 0);
			break;
		    case t_float:
...
void EiC_do_stooutput(token_t * e1)
{
	...
	/* EiC-Improved only: volatile lval must be accessed through referencing */
	if(!__backward__ && t >= t_char && t <= t_ullong && isvolatile(e1->Type)) 
	  EiC_warningerror("Suspicious lval with volatile qualifier");
	...
	switch (EiC_gettype(e1->Type)) {
	  ...
	  CASE_ULONG:EiC_generate(&e1->Code, stolong, &e1->Val, EiC_GI(e1));
	    break;
	  case t_llong:
	  case t_ullong:EiC_generate(&e1->Code, stollong, &e1->Val, EiC_GI(e1));
	    break;
	  ...
	  case t_lval:
	    ...
	    switch (EiC_gettype(e1->Type)) {
	      case t_uchar:
	      case t_char:
		EiC_generate(&e1->Code, volqual ? refvolchar : refchar, &e1->Val, 0);
		break;
	      case t_short:
	      case t_ushort:
		EiC_generate(&e1->Code, volqual ? refvolshort : refshort, &e1->Val, 0);
		break;
	      case t_int:
	      case t_uint:
		EiC_generate(&e1->Code, volqual ? refvolint : refint, &e1->Val, 0);
		break;
	      CASE_LONG:
	      CASE_ULONG:
		EiC_generate(&e1->Code, volqual ? refvollong : reflong, &e1->Val, 0);
		break;
	    case t_llong:
	    case t_ullong:
	      EiC_generate(&e1->Code, volqual ? refvolllong : refllong, &e1->Val, 0);
		break;
	      case t_float:
...
void binhconst(unsigned opr, token_t * e1, token_t * e2)
{
    ...
    switch (t) {
      ...
      case t_ullong:f = binhullong; break;
