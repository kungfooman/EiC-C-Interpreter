...
static void *vol_retp;
static int vol_ret;

#define drefVolTYPE(x,y)  checkPtr(STK[ToP].v.p,"READ", sizeof(x) );\
            vol_retp = link2eic_read(STK[ToP].v.p.p, sizeof(x)); \
            if(vol_retp == NULL) { \
        	  EiC_messageDisplay("READ: access failed\n");\
              raise(SIGBUS); } \
            else STK[ToP].v.y = *(x*)vol_retp

			/**  STK[ToP].v.y = *(x*)STK[ToP].v.p.p **/



#define refVolTYPE(x,y)   ToP--;\
		    checkPtr(STK[ToP].v.p,"WRITE", sizeof(x) );\
            vol_ret = link2eic_write(STK[ToP].v.p.p, sizeof(x), (void *)&STK[ToP+1].v.y); \
            if(vol_ret != 0) { \
        	  EiC_messageDisplay("WRITE: access failed\n");\
              raise(SIGBUS); \
            } 
...
void EiC_interpret(environ_t * env)
{
	...
	if(EiC_traceON) {
		...
		EiC_formatMessage("\n[%s] ",(char*)v.p.p);
		/* trace info is forwarded to the agent */
		if(!__backward__) {
		    sprintf(msg, "('trace', 'func: %s',)\n", (char*)v.p.p);
		    send_pipe(msg);
		}
	    }
	...
	/* volatile storage */
	case drefvolushort: drefVolTYPE(unsigned short, uival); break; 
	case drefvolshort: drefVolTYPE(short, ival); break;
	case refvolshort: refVolTYPE(short, ival); break;
	case stoshort:
	...
	case stochar:
	    ...
	    break;
	case drefuchar: drefTYPE(unsigned char, uival); break;
	case drefchar: drefTYPE(char, ival); break;
	case refchar: refTYPE(char, ival); break;
	case drefvoluchar: drefVolTYPE(unsigned char, uival); break;
	case drefvolchar: drefVolTYPE(char, ival); break;
	case refvolchar: refVolTYPE(char, ival); break;
	...
	case drefvolint: drefVolTYPE(int, ival); break;
	case refvolint: refVolTYPE(int, ival); break;
	...
	case drefvoluint: drefVolTYPE(unsigned, uival); break;
	case refvoluint: refVolTYPE(unsigned, uival); break;
	...
	case drefvollong: drefVolTYPE(long, lval); break;
	case refvollong: refVolTYPE(long, lval); break;
	...
	case drefvolulong: drefVolTYPE(unsigned long, ulval); break;
	case refvolulong: refVolTYPE(unsigned long, ulval); break;
	...
	case drefvolllong: drefVolTYPE(eic_llong, llval); break;
	case refvolllong: refVolTYPE(eic_llong, llval); break;
	...
	case llong2long: castTYPES(llval, lval, long); break;
	  ...
	  /* specific unsigned long long stuff */
	case incullong: STK[ToP].v.ullval += InSt[p].val.ival; break;
	case decullong: STK[ToP].v.ullval -= InSt[p].val.ival; break;
	case drefullong: drefTYPE(eic_ullong, ullval); break;
	case refullong: refTYPE(eic_ullong, ullval); break;
	case drefvolullong: drefVolTYPE(eic_ullong, ullval); break;
	case refvolullong: refVolTYPE(eic_ullong, ullval); break;
	case stoullong: stoTYPE(ullval); break;
	case rvalullong: rvalTYPE(ullval); break;
	case pushullong: pushTYPE(ullval); break;
	case negullong: negTYPE(ullval); break;
	case addullong: addTYPE(ullval); break;
	case subullong: subTYPE(ullval); break;
	case divullong: divTYPE(ullval); break;
	case multullong: multTYPE(ullval); break;
	case modullong: modTYPE(ullval); break;
	case lshtullong: lshtTYPE(ullval); break;
	case rshtullong: rshtTYPE(ullval); break;
	case ltullong: ltTYPE(ullval); break;
	case leullong: leTYPE(ullval); break;
	case equllong: eqTYPE(ullval); break;
	case neullong: neTYPE(ullval); break;
	case gtullong: gtTYPE(ullval); break;
	case geullong: geTYPE(ullval); break;
	case lorullong: lorTYPE(ullval); break;
	case landullong: landTYPE(ullval); break;
	case notullong: notTYPE(ullval); break;
	case borullong: borTYPE(ullval); break;
	case xorullong: xorTYPE(ullval); break;
	case andullong: andTYPE(ullval); break;
	case compullong: compTYPE(ullval); break;
	case ullong2double: castTYPES(ullval, dval, double); break;
	case ullong2ptr: castTYPES(ullval, p.p, void *); break;
	case ullong2int: castTYPES(ullval, ival, int); break;
	case ullong2long: castTYPES(ullval, lval, long); break;
...
*/
	...
	case eicreturn:
		...
		/* trace info is forwarded to the agent */
		if(!__backward__) {
			/* finding the line number the control is returning to */
			next_p = p+1;
			/* skipping instructions without line info */
			while(InSt[next_p].line == 0) next_p += 1;
		    sprintf(msg, "('trace', 'resume: %s @ %d',)\n", (char*)v.p.p, InSt[next_p].line);
		    send_pipe(msg);
		}
	    }
	    break;
	...
	case checkar:{
	    ...
	    break;
	case bgjob:
		  /* raise the signal if any bits are set */
	      if(link2eic_bgjob(InSt[p].val.ival) > 0) raise(SIGUSR2);
	    break;
