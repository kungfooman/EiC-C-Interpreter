...
static void stmt1(token_t * e1)
{
    ...
    /* invoke bgjob(0) between stmts */
    /* no need to place bgjob back to back */
    if(!__backward__ && e1->Code.nextinst > 0 && opcode(&e1->Code, e1->Code.nextinst-1) != bgjob) {
      v.ival = 0;
      EiC_generate(&e1->Code, bgjob, &v, 0);
	  setCodeLineNo(&e1->Code,e1->Code.nextinst-1,0);
    }
...
static void f_cast_expr(token_t * e1)
{
	...
	if (isconst(e2.Type)) {
	    ...
	    e1->Val = e2.Val;
	    e1->Tok = e2.Tok;	/* propagate the Tok */
	} else {
	    EiC_output(&e2);
	    EiC_castvar(&e2, e1, 1);
	    e1->Pflag = e2.Pflag;
...
static void primary_expr(token_t * e1)
{
	...
	*e1 = *token;
	...
	break;
      case LLONG:
	*e1 = *token;
	e1->Type = EiC_addtype(t_llong, NULL);
	setConst(e1->Type);
	break;
      case ULLONG:
	*e1 = *token;
	e1->Type = EiC_addtype(t_ullong, NULL);
	setConst(e1->Type);
	break;
		    ...
		    token_t * e1, int op)
{
	...
	*e1 = e2;
    ...
    if (!(e1->Pflag||e2.Pflag) && isconst(e1->Type) && isconst(e2.Type)) {
	EiC_contoken(e1, &e2);
	e1->Pflag = 0;
	return;
    }
