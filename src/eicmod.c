...
static void outputval(int obj, void *v)
{
    if(EiC_radixBase == 10) {
      ...
      switch (obj) {
        ...
        case t_uchar:   fprintf(stdout,"%u",*(unsigned char *) v); break;
        case t_short:   fprintf(stdout,"%d",*(short *) v); break;
        case t_ushort:  fprintf(stdout,"%u",*(unsigned short*) v); break;
        ...
        case t_llong:   fprintf(stdout,"%lld",*(eic_llong *) v); break;
        case t_ullong:  fprintf(stdout,"%llu",*(eic_llong *) v); break;
#else
        case t_llong:   fprintf(stdout,"%ld",*(eic_llong *) v); break;
        case t_ullong:  fprintf(stdout,"%ld",*(eic_llong *) v); break;
#endif 
      ...
      /* in HEX */
      switch (obj) {
        case t_char:    fprintf(stdout,"0x%x",*(char *) v);break;
        case t_uchar:   fprintf(stdout,"0x%x",*(unsigned char *) v); break;
        case t_short:   fprintf(stdout,"0x%x",*(short *) v); break;
        case t_ushort:  fprintf(stdout,"0x%x",*(unsigned short*) v); break;
        case t_int:     fprintf(stdout,"0x%x",*(int *) v); break;
        case t_uint:    fprintf(stdout,"0x%x",*(unsigned *) v); break;
        case t_long:    fprintf(stdout,"0x%lx",*(long *) v); break;
        case t_ulong:   fprintf(stdout,"0x%lx",*(long *) v); break;

#ifndef NO_LONG_LONG

        case t_llong:   fprintf(stdout,"0x%llx",*(eic_llong *) v); break;
        case t_ullong:  fprintf(stdout,"0x%llx",*(eic_llong *) v); break;
#else
        case t_llong:   fprintf(stdout,"%ld",*(eic_llong *) v); break;
        case t_ullong:  fprintf(stdout,"%ld",*(eic_llong *) v); break;
#endif 
...
static void EiC_showdectype(type_expr * t, int expand, FILE *fp)
{
	...
	switch (EiC_gettype(t)) {
	  ...
	  case t_ullong:fputs("unsigned long long ",fp); break;
...
static long getmitem(val_t *v, int t)
{
    switch(t) {
      ...
      case LLONG: return (long)v->llval;
      case ULLONG: return (long)v->ullval;
    }
    return -1;      
}
...
static int toggle_radix()
{
    EiC_radixBase = EiC_radixBase == 10 ? 16 : 10;
    return EiC_radixBase;
}
...
puts("-----------------------------------------------------------------------------\n"
     ...
     ":radix         Switch the radix (base=10/16) to be used.\n"
...
static void genAffect(type_expr * t, int expand, FILE *fp)
{
	...
	switch (EiC_gettype(t)) {
	  ...
	  case t_llong:  fputs("v.llval = ", fp); break;
	  case t_ullong: fputs("v.ullval = ", fp); break;
	  case t_float: 
...
void EiC_init_EiC(void)
{
    ...
    add_eicfunc("radix",toggle_radix);
