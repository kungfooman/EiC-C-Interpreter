...
int _eicUprintf(int (*output)(), void *arg, char *fmt, arg_list ap)
{
	...
	switch(*fmt++) {
	  case 'c': sptr[0] = (char)nextarg(ap,int);
	    ...
	    break;
	  case 'b':
	    if(llseen) ulltoa(nextarg(ap,long long),sptr,2);
	    else if(lseen) ultoa(nextarg(ap,long),sptr,2);
	    else utoa(nextarg(ap,int),sptr,2);
	    break;
	  case 'i':
	  case 'd': 
	  if(llseen) lltoa(nextarg(ap,long long),sptr,10);
	  else if(lseen) ltoa(nextarg(ap,long),sptr,10);
	  else itoa(nextarg(ap,int),sptr,10);
	  dprec=1;
	    break;
	  case 'u': 
	  if(llseen) ulltoa(nextarg(ap,unsigned long long),sptr,10);
	  else if(lseen) ultoa(nextarg(ap,unsigned long),sptr,10);
	  else utoa(nextarg(ap,unsigned),sptr,10);
	  dprec=1;
	    break;
	  case 'o':	if(mod) *sptr = '0';
	    if(llseen)lltoa(nextarg(ap,long long),&sptr[mod],8);
	    else if(lseen)ltoa(nextarg(ap,long),&sptr[mod],8);
	  ...
	  case 'x': case 'X':
		...
		*sptr = '0', sptr[1] = *(fmt-1);
	    ...
	    if(llseen)ulltoa(nextarg(ap,long long),&sptr[mod],16);
	    else if(lseen)ultoa(nextarg(ap,long),&sptr[mod],16);
    ...
    while(*fmt != '\0') {
        if(isspace(*fmt)) {  /* match white space */
            while(*fmt && isspace(*fmt)) 
              fmt++;
                             /* read input UPTO next non-whitespace; */
        ...
        switch(*fmt) {
          ...
          case '[':
               ...
               switch(*fmt)
                 {
                  case '-' :
                    ...
                    if ((*fmt == ']') || 
                        (ssai == 0) ||
                        (scansetarray[ssai-1] > *fmt))
                      /*'-' stands for itself, just add '-' */
                      scansetarray[ssai++] = '-';
                    else 
                      {
                       /* fill in missing components */
                       while(scansetarray[ssai-1] != *fmt)
                         {
                          scansetarray[ssai] = scansetarray[ssai-1] + 1;
                          ssai++;
                         }
                       /* advance fmt to the next char  in fmt*/
                       fmt++;
                      }
                     break;
                  default  : 
            ...
            if(ssai == 0) return EOF;
            carg = vptr;
            while((ch = (*input)(arg)) != EOF) 
                if(type) {
                    if(memchr(scansetarray,ch,ssai)) {
                        if(!wast)
                            *carg++ = ch;
                        ++cnt;
                    }else {
            ...
            *p = '\0';
        {
            ...
            if(*fmt == 'd' || *fmt == 'i') { /* signed conversion */
                ...
                long long llval = strtoll(field,&endptr,base);
                if(type) {
                    if(type == 'h')
                        ...
                        *(long long *)vptr = llval;
                } else
                    *(int *)vptr = (int) lval;
                        
            } else {
                unsigned long ulval = strtoul(field,&endptr,base);
                unsigned long long ullval = strtoull(field,&endptr,base);
                if(type) {
                    if(type == 'h')
                        ...
                        *(unsigned long long *)vptr = ullval;
                } else
