...
#include <stdlib.h>
#else
/* this is to access lldiv_t under Linux */
#define __USE_ISOC99
#include <stdlib.h>
#undef __USE_ISOC99
#endif
...
val_t eic_lldiv(void)
{
    val_t v;
    static lldiv_t d; /* create some memory */

    /* skip argument -1 & start at -2 */

    long long num = arg(1,getargs(),long long);
    long long dem = arg(2,getargs(),long long);
...
void module_math(void)
{
    ...
    EiC_add_builtinfunc("lldiv",eic_lldiv);
