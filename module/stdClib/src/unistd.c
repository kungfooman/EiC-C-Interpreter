...
static val_t eic_getcwd(void)
{
	...
	v.p.sp = v.p.p;
	v.p.ep = (char*)v.p.p + strlen(v.p.sp);
    } else
	v.p.p = getcwd(v.p.p, arg(1,getargs(),size_t));
...
static val_t eic_getlogin(void)
{
    val_t v;

    v.p.sp = v.p.p = getlogin();
    v.p.ep = (char*)v.p.p + strlen(v.p.sp);
    return v;
}


static val_t eic_usleep(void)
{
    val_t v;
    
    usleep(arg(0,getargs(),unsigned long));
...
void module_unistd(void)
{
    ...
    EiC_add_builtinfunc("getlogin",eic_getlogin);
    EiC_add_builtinfunc("usleep",eic_usleep);
