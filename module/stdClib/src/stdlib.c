...
/* long long (64-bit) support */

val_t eic_strtoll(void)
{
    val_t v;
    arg_list ap = getargs();
    ptr_t p1, *p2;
    ...
    v.llval = strtoll(p1.p,
		    endptr,
		    arg(2,ap,int));
    return v;
}

val_t eic_strtoull(void)
{
    val_t v;
    arg_list ap = getargs();
    ptr_t p1, *p2;
...
val_t eic_lltoa(void)
{
    ...
    switch(arg(3,ap,int)) {
      case 1:
	v.p.sp = v.p.p = lltoa(arg(0,ap,long long),
		      v.p.p,
		      arg(2,ap,int));
	break;
      case 2:
	v.p.sp = v.p.p = ulltoa(arg(0,ap,unsigned long long),
		       v.p.p,
		       arg(2,ap,int));
	break;
    }
...
val_t eic_atoll(void)
{
    val_t v;
    v.llval = atoll(nextarg(getargs(),ptr_t).p);
    return v;
}
...
void module_stdlib(void)
{
    ...
    EiC_add_builtinfunc("_lltoa", eic_lltoa);
    ...
    EiC_add_builtinfunc("strtoll", eic_strtoll);
    EiC_add_builtinfunc("strtoull", eic_strtoull);
    ...
    EiC_add_builtinfunc("atoll", eic_atoll);
